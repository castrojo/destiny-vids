...
- **Kelsey Hightower has no deck entry** (#33). Zavala's binding plates gold —
  chrome claims nothing about anybody — but the eyebrow, subclass and seal rows
  are omitted with a `TODO(owner)` rather than borrowed from the character. A
  lead's `class: titan` tags describe *Zavala*; printing that on the card would
  make it a claim about Kelsey, which only the owner or the deck may make.
