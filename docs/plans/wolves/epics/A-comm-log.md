...
- An unknown `@login` is a hard error. Never drop a line to keep a parse alive.
...
are a `warn`; a chatter line whose login is not resolvable is a `fatal` (A4 does
the resolving — here, just record the login).
