...
- A missing or unverifiable logo is a **failure, not a silent skip**. Dropping an
  org's slide because its asset 404'd is the credit-dropping failure mode rule 3
  exists to prevent.

**Acceptance:**
- [ ] Every logo in the manifest has a source, a checksum and a permission basis.
- [ ] No image asset is committed to the repo.
- [ ] The intro deck renders from the manifest, and a missing asset fails loudly.
