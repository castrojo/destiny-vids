...
**Sequence position: 02.** The second cut in the ordered upload series, so it
ships as `02-zavala` everywhere: outline, cut list, plate manifest, renders.

| Piece | Where |
|---|---|
| Outline | [`stories/02-zavala.txt`](../../stories/02-zavala.txt) |
...
```bash
python3 tools/story.py stories/02-zavala.txt --dir segments \
...
In [`stories/02-zavala.txt`](../../stories/02-zavala.txt), nowhere else. Each
...
2. Insert the beat line into `stories/02-zavala.txt` **in source-time order**,
...
It carries **one text row**: `name: Kelsey Hightower`, which is the binding's
own `display_name`. The eyebrow, the subclass and the seal are **omitted**,
because Kelsey has no entry in the reference deck (`~/Videos/nameplates.json`)
and nobody has authored those rows for him. Borrowing a subclass from Zavala's
own `class: titan` tags would be a lore call about the character wearing a real
person's credit — the exact fault the Osiris binding's comment warns about — so
the card ships three rows short with a `TODO(owner)` beside it. Chrome is safe
to choose because it claims nothing about anybody; copy is not.
...
python3 tools/render.py cut.json --media media --out renders/02-zavala-web.mp4 \
    --max-shot-sec 9
...
python3 tools/render.py cut.json --media media \
    --out renders/02-zavala-youtube.mp4 --max-shot-sec 9

# 4. the gold nameplate, burned into whichever output needs it
python3 tools/plate.py burn --video renders/02-zavala-youtube.mp4 \
    --manifest plates.json --out renders/02-zavala-youtube-plated.mp4
```
...
**Sequence order lives in the filename prefix.** `02-` is the upload position,
after `01-dance`; the next cut is `stories/03-<name>.txt` →
`renders/03-<name>-*.mp4`. Sorting the directory sorts the playlist. There is
...
| Kelsey Hightower's eyebrow, subclass and seal rows | Plate copy is a claim about a real person, and the reference deck authorises none of these for him. The rows are omitted, with `TODO(owner)` in `vocab/casting.yaml`. |
