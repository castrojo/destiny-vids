...
but it is no longer uncredited. Fifteen name plates credit the people the show is
...
Forty-eight cards in all — eighteen name plates, twenty-six dialogue pills, a
chapter card, the patch-queue HUD, the villain's bar and the letterbox
callout — generated by `scripts/build_efmb_plates.py` into
...
**hanthor** (3:30.034) and **Ahmed Adan** (3:37.701). Only the *scheduling*
went: Rizzo's authored copy is still in `vocab/casting.yaml` and Ahmed is still
in `ensemble.placeholders`, and all four are recorded in the manifest's
`unresolved` because nobody has decided where they are credited instead.

**The marks came off a different clock, and that was proved rather than
assumed.** Issue #98's timecodes are act II **film** time; this round's are
**megacut** time, where act II sits at **+2:01.567**. The proof is a frame: at
megacut 5:35 the burned film still carries hanthor's plate (act II 3:33.4), and
act II's own 4:59 is black tail, so the standalone reading is impossible. The
conversion table is in `scripts/build_efmb_plates.py`, and every anchor there
is a **source** timecode like every other window in this act.

Two of the owner's marks are pinned exactly — "turn the stream on" at film
177.433 and "There's nothing glorious about this job" at 186.433 — and a test
fails if the chain of cues before them ever pushes one late. His 5:35 is the
one mark deliberately **not** honoured: it is 0.9 s before the cut, and the
villain's bar goes on the frame the villain is actually on (3:34.734).

Three chrome things this needed, all of them colour and imagery rather than a
new row of words: **`variant: nobara`**, sampled from the official
`nobaraproject.org` icon (#3E3FC5, the brand's own Governor Bay);
**`variant: youtube`** for a creator whose affiliation is his channel; and
`tools/plate.py` gained the **`miniboss`** bar (Destiny's boss-bar treatment in
the red the owner asked for) and the **`achievement`** toast.

**The achievement gag is built and not burned.** The owner asked to approve the
strings first, so `WALK_ACHIEVEMENTS_APPROVED` is `False`, the four measured
explosions and the proposed copy sit in `unresolved`, and only *"Mailing List
Bullshit"* is his. A proposal that quietly rendered would be an invented line
on screen.

One bug fell out of it: **every chat pill in this act was rendering with the
drawn crest instead of a pfp**, because the builder handed the avatar resolver
an empty dict. The walk's pills now carry their speaker's picture; the
montage's two Jorge Castro pills still do not, because `castrojo` is a lead and
no avatar is recorded on the `cayde_6` binding — recorded, not guessed.
...
### The announcement overlay, the TOC exchange and the endgame (#98)

The first-alpha brief for this act has four parts; every word of copy is the
owner's, reproduced verbatim, and every timecode is **act II film time** —
the clock trap of [#109](../../docs/running-order.md) applies, and every
owner mark is anchored to a source instant so a cut that moves raises instead
of sliding.

**§1, the montage announcements** (1:38 → 2:19): two pfp asides, then the
heraldic ranks — bronze greets the newcomer, silver flatters the incumbent,
gold is kept for the emeriti and the payoff. `bronze` was the one new chrome
this needed, beside `leader` (gold) and `trustee` (silver). Delivered.

**§2, the 2:19 lead-in banner** — **not built**: the one cue he called
important is the one cue with no copy written. Recorded in `unresolved`.

**§3, the TOC exchange** (the CNCF exchange: Karena, Joseph, Ricardo). His
5:07 for Joseph's last line is inside the 16.065 s black tail, and his ruling
put the exchange in the greenery — which is where *The Long Walk* lives. So
it is laid out **around** the walk: the three questions play 2:19 → 2:27.6
(with the trio itself on screen), the answer lands 3:41 → 4:00.5 after the
walk's last card, and Karena's jump is **1.5 s of clear screen** between
Joseph's `DO` line and Ricardo's answer — no card, per the brief. The payoff
**reprises** the montage's emeritus card verbatim (a callback, so the
double-credit guard allows an identical repeat) and pivots to *"Have you met
our Ambassadors?"*.
...
The nine names the brief adds that no vocab binds (`AN4-CH3CK-12`, `krook`,
`Natewaddington`, `cgwalters`, `siosm`, `jberkus`, `preethi`, plus
`cncf marketing` and the `Open Gaming Collective` callout) render as
name-only placeholder badges with the drawn crest, and every one is recorded
in `unresolved` — logins are confirmed by the owner, never guessed.
...
0:54.234  TOC
2:27.801  The Long Walk
```

"Rizzo" went with Rizzo's plate: a marker that drops the audience on a credit
that is no longer there points at nothing.
