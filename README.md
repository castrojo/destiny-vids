...
[`docs/running-order.md`](docs/running-order.md) for what the show is, then
[`docs/SKILL.md`](docs/SKILL.md) to load the one skill your task needs.

## What this repo produces: the feature and hero videos
...
whole unit** at KubeCon NA on **9 Nov 2026**. It is **eight acts behind a
prologue**, and [`docs/running-order.md`](docs/running-order.md) is the source
of truth for what they are and what order they play in. Act VI is the musical —
one song end to end, cut to Nightwish's *7 Days to the Wolves* — the longest
act and the centre of the show, but not the whole show. Every act has a film
...
in the run-up. The binding constraint is **indexing, not editing** — only two
cast members currently have enough footage
([issue #50](https://github.com/castrojo/destiny-vids/issues/50)).

**Appearing in the feature is not somebody's hero video.** Act IV is Kat's act
*and* her hero video is a separate, separately-released thing.
...
**A cut is not documented in prose.** Its outline lives in `stories/`, its
...
Two rules are stronger than a preference, and both live in the vocabulary rather
than in an outline:

- **The Witness is shown as eyes or smoke, never its body.** A `depiction` rule
  on the binding, defaulting to exclusion — see
  [`docs/skills/casting/SKILL.md`](docs/skills/casting/SKILL.md).
...
[`docs/skills/production/references/delivery.md`](docs/skills/production/references/delivery.md).
