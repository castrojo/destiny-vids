...
whole unit** at KubeCon NA. It is a **musical**: one song end to end, in three
acts, cut to Nightwish's *7 Days to the Wolves*. A prototype is staged
([`docs/cuts/07-seven-days-to-the-wolves.md`](docs/cuts/07-seven-days-to-the-wolves.md));
it still needs a credits sequence and an owner decision on footage provenance.

It used to be four parts in a running order. It is not any more, so the Europa
director's cut and the Nati teaser are **unplaced** pending a decision — see
...
| `scripts/build_wolves.py` | Builds the feature's authored shotlist: three acts, hinged on the bed's measured anchors, with the artwork cards. |
...
| 07 — Seven Days to the Wolves (the feature) | [`docs/cuts/07-seven-days-to-the-wolves.md`](docs/cuts/07-seven-days-to-the-wolves.md) |
