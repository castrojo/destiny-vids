...
# The Zavala cut (docs/cuts/02-zavala.md) comes out of the same cinematic, and
# is the one-cinematic mechanic pointed at a single lead instead of the
# ensemble.

def _zavala_story(indexed):
    _, _, outline = read_outline(str(REPO_ROOT / "stories" / "02-zavala.txt"))
...
def test_zavala_cut_uses_every_clean_zavala_shot_this_cinematic_has(indexed):
    ...
    toward a later reveal by finding more of him (see docs/cuts/02-zavala.md).
    """
    _, story = _zavala_story(indexed)
