...
def test_source_time_maps_through_the_blueberries_excision():
    cuts = [{"start_sec": 44.52, "end_sec": 54.68}]
    assert standalone.source_to_output(44.0, cuts) == 44.0
    assert standalone.source_to_output(97.0, cuts) == pytest.approx(86.84)
...
def test_kept_ranges_remove_exactly_the_authored_span():
    assert standalone.kept_ranges(
        120.0, [{"start_sec": 44.52, "end_sec": 54.68}]
    ) == [(0.0, 44.52), (54.68, 120.0)]
...
def test_overlay_source_marks_are_mapped_before_render():
    ...
    assert overlays[0]["at"] == pytest.approx(49.84)
    assert unresolved == []
...
def test_expected_output_duration_removes_every_cut():
    assert standalone.expected_duration(
        {"cuts": [{"start_sec": 44.52, "end_sec": 54.68}]}, 120.0
    ) == pytest.approx(109.84)
...
def test_the_takeover_starts_before_the_new_legends_title():
    ...
    silhouette"). The takeover at 91.7 -- output 81.54 after the 10.16s
    ...
    assert video["cuts"][0]["start_sec"] == 44.52
    assert video["cuts"][0]["end_sec"] == 54.68
    assert video["cuts"][0]["end_sec"] - video["cuts"][0]["start_sec"] == \
        pytest.approx(10.16)
    assert standalone.source_to_output(
        video["takeover"]["source_at"], video["cuts"]) == pytest.approx(81.54)
