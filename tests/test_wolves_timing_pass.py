...
def test_film_is_exactly_its_song(cut):
    """Since 12f4ebb the film IS the record, end to end.

    The interruption (issue #104) was the only thing that ever made wall
    longer than bed; the owner ordered it out of act VI ("get rid of it lol
    this moved to the endless song"), and the block was free by construction
    -- the bed clock never advanced across it. So the two clocks are now one:
    any gap between them is a pause nobody authored.
    """
    regions = plan_regions(cut["shots"], cut["bed_offset_sec"])
    wall = sum(s["duration"] for s in cut["shots"])
    assert wall == pytest.approx(total_bed(regions), abs=0.02)
...
def _interruption(cut):
    """Nothing: the interruption was removed on the owner's order (12f4ebb).

    Kept as the single place that says so, so the tests below read as
    statements about the current cut rather than as missing coverage. The
    removed block was wall 322.200-341.670 -- 1.0 s held silence, the 4.0 s
    Ambassadors slide, the 4.0 s Amber plate, the 10.470 s clip -- all of it
    free to the song: the bed clock never advanced across it.
    """
    beats = [s for s in cut["shots"] if "INTERRUPTION" in s["beat"]]
    assert beats == [], (
        "the interruption is back -- the owner ordered it out of act VI "
        "('get rid of it lol this moved to the endless song', 12f4ebb); "
        "Amber is presented in act II now")
    return beats


def test_the_interruption_consumes_no_bed_time(cut):
    """There is nothing left to consume bed time: every beat is the song."""
    _interruption(cut)
    dispositions = {s.get("audio", "bed") for s in cut["shots"]}
    assert dispositions == {"bed"}, (
        "a non-bed beat (silent/hold/source) is a pause in the song, and the "
        "only pause act VI ever had went out with the interruption")
    regions = plan_regions(cut["shots"], cut["bed_offset_sec"])
    assert all(r["kind"] == "bed" for r in regions)
...
def test_no_beat_plays_its_own_score(cut):
    """The gameplay clip was the cut's only source-audio moment, and it went
    with the interruption (12f4ebb). A new `audio: "source"` beat mutes the
    bed under itself -- a pause in the song nobody authored."""
    assert [s for s in cut["shots"] if s["audio"] == "source"] == []
...
def test_cortney_is_intro_only_and_nobody_owns_a_removed_interruption(cut):
    """Cortney's only credit is Act I, and Amber is presented in act II now:
    the sequence she owned here was removed on the owner's order (12f4ebb),
    so no beat in this cut may name her. The c-amber card stays in
    stories/06-wolves-interruption-cards.json as the record of what played;
    the shotlist simply never asks for it."""
    assert all("Amber" not in shot["beat"] for shot in cut["shots"])
    assert all("Cortney" not in shot["beat"] for shot in cut["shots"])


def test_the_hold_slot_went_with_the_interruption(cut):
    """The hold-music slot existed only inside the interruption (B and C
    carried it). With the interruption removed (12f4ebb) there is no hold
    beat left to fill; the cleared track (Local Forecast - Slower, CC BY
    4.0, credited in ATTRIBUTIONS.md) moved to act II with Amber. A hold
    beat reappearing here is the slot coming back, not a licensing find.
    """
    assert [s for s in cut["shots"] if s["audio"] == "hold"] == []
...
def test_the_builder_asks_for_no_interruption_cards(cut):
    """The cards manifest (stories/06-wolves-interruption-cards.json) stays
    on disk as the record of what played, but since 12f4ebb no shot may
    reference it: the builder asks for none of it. A still pointing into
    renders/interruption/ is the interruption coming back.
    """
    for shot in cut["shots"]:
        still = Path(str(shot.get("still", "")))
        assert still.parent.name != "interruption", (
            f"{still}: an interruption card is back in the cut")
...
def test_the_clip_the_owner_named_is_gone_with_the_interruption(cut):
    """The explosion/portrait clip (source 43.000 -> 53.470, measured for
    #104) was the interruption's payload. The owner moved the whole moment
    to act II (12f4ebb), so no shot here may come from the gameplay trailer
    window it occupied."""
    assert [s for s in cut["shots"] if s["audio"] == "source"] == []
    assert not any("INTERRUPTION" in s["beat"] for s in cut["shots"])
